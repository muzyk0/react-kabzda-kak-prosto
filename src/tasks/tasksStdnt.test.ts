...
test('func return sum numbers', () => {

    expect(funcToPropsNum(3, 5, 7, 6, 4, 9)).toBe(34)
})

test('get Triangle Type', () => {
    expect(getTriangleType(1, 1, 1)).toBe('10') // Равносторонний
    expect(getTriangleType(3, 3, 2)).toBe('01') // Равнобедренный
    expect(getTriangleType(5, 4, 3)).toBe('11') // Обычный
    expect(getTriangleType(10, 5, 5)).toBe('00') // Не сущестует
})
